...
export function FromBuffer(buffer: Buffer): [u: Universal, b: Buffer] {
    if (buffer.length < 2) {
        return [null, buffer]
    }
    try {
...
export function FillConnect(u: Universal): Connect {
    if (u.commandType != "C") {
        return null
    }
    let p = MakeConnect()
...
export function FillSubscribe(u: Universal): Subscribe {
    if (u.commandType != "S") {
        return null
    }
...
export function FillSend(u: Universal): Send {
    if (u.commandType != "P") {
        return null
    }
    let p: Send = MakeSend()
...
export function FillLookup(u: Universal): Lookup {
    if (u.commandType != "L") {
        return null
    }
